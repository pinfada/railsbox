...
test("T2 — une REDIRECTION SUIVIE passe le test de mise en cache : le défaut", async () => {
  ...
  // page de connexion, et la condition en ligne de serveArtifact la juge
  // cacheable. La page de connexion est alors écrite en Cache Storage SOUS
  // L'URL DU MORCEAU DE DISQUE, en « cache d'abord » sans revalidation :
  // après reconnexion, la sandbox reste cassée jusqu'à la reconstruction
  // suivante (le nom du cache ne change qu'avec `builtAt`).
  //
  // Ce test ÉCHOUERA — et devra être inversé — quand `estArtefactCacheable`
  // fermera le trou. C'est le but : il chiffre le défaut au lieu de le
  // décrire.
  ...
  try {
    ...
    await worker.viderDifferes();
    assert.equal(
      worker.misEnCache.length,
      1,
      "défaut mesuré : une page de connexion redirigée EST mise en cache",
    );
    assert.equal(worker.misEnCache[0].url, MORCEAU, "…sous l'URL du morceau de disque");
  } finally {
    worker.fermer();
  }
});
