...
test("un verrou Yarn Classic est INSTALLÉ avec yarn, et n'est plus signalé", async () => {
  // Avant, railsbox retombait sur npm et se contentait d'avertir. Ce repli
  // n'était pas une dégradation pour woofed-crm : npm REFUSE son arbre
  // (ERESOLVE), donc la construction échouait. La génération se lit sur le
  // CONTENU du verrou — ce test l'exerce à travers detectApp, donc à travers
  // la vraie lecture de fichier, pas seulement la fonction pure.
  const dir = await createApp({
  ...
  assert.equal(manifest.assets.manager, "yarn");
  assert.equal(manifest.assets.install, "yarn install --frozen-lockfile");
  assert.equal(findByCode(findings, "npm-lockfile-absent"), undefined);
});

test("un verrou Yarn moderne sans `packageManager` retombe sur npm, en le disant", async () => {
  // Corepack n'aurait aucune version à provisionner, et sa valeur par défaut
  // est un Yarn 1 — qui refuserait ce verrou. Mieux vaut npm et un
  // avertissement qu'une commande fausse au milieu d'une construction.
  const dir = await createApp({
    "Gemfile.lock": lockWith(["cssbundling-rails"]),
    "package.json": JSON.stringify({ scripts: { "build:css": "sass x y" } }),
  ...
  assert.equal(findByCode(findings, "yarn-sans-package-manager").severity, "warning");
});
