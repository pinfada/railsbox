...
| **Chaînes npm** (esbuild, cssbundling) | **pris en charge** par le même étage (`npm ci` puis scripts de build). **pnpm** est reconnu quand `packageManager` le déclare ; un verrou pnpm SANS cette clé retombe sur npm, avec un avertissement. **yarn** est exécuté quand son verrou dit sa génération — `# yarn lockfile v1` (Classic, `--frozen-lockfile`) ou `__metadata:` (Berry, `--immutable`) ; Berry exige en plus un `packageManager`, faute de quoi Corepack retomberait sur un Yarn 1 qui refuse ce verrou. bun est **signalé, pas exécuté** : deux verrous contradictoires arrêtent la construction. |
