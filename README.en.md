...
**Browsers**: validated on Chromium. Webviews that block Service Workers cannot
...
authenticated (`auto_login`), and the bill stays at zero on the day your link
hits Hacker News. The non-negotiable trade-off: **nothing real may be shipped
...
| **Tailwind, dart-sass** | **supported**: precompiled on an amd64 stage, then copied into the i386 disk (the guest never runs those binaries). Validated at the stage itself; an end-to-end boot in CI is still pending. |
...
│                                  + its PostgreSQL overlay (demo-pg/)
